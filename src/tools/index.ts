...
export async function registerAllTools(server: McpServer): Promise<void> {
  ...
  server.registerTool('get_markets', {
    ...
    annotations: RO,
  }, withTimestamp(getMarketsHandler, 60));
  ...
  server.registerTool('get_odds', {
    ...
    annotations: RO,
  }, withTimestamp(getOddsHandler, 5));
  ...
  server.registerTool('get_orderbook', {
    ...
    annotations: RO,
  }, withTimestamp(getOrderbookHandler, 3));
  ...
  server.registerTool('search_markets', {
    ...
    annotations: RO,
  }, withTimestamp(searchMarketsHandler, 60));

  server.registerTool('get_whale_positions', {
    title: 'Get Whale Positions',
    description: 'Positions of a specific Polymarket wallet, optionally filtered to one market. The /positions API requires a user (no public "all holders by market" endpoint), so pass the wallet address of the trader you want to inspect.',
    inputSchema: {
    ...
    annotations: RO,
  }, withTimestamp(getWhalePositionsHandler, 60));
  ...
  server.registerTool('get_movers', {
    ...
    annotations: RO,
  }, withTimestamp(getMoversHandler, 30));
  ...
  server.registerTool('get_markets_near_resolution', {
    ...
    annotations: RO,
  }, withTimestamp(getMarketsNearResolutionHandler, 60));
  ...
  server.registerTool('get_volume_spikes', {
    ...
    annotations: RO,
  }, withTimestamp(getVolumeSpikesHandler, 30));
  ...
  server.registerTool('get_late_game_sports', {
    ...
    annotations: RO,
  }, withTimestamp(getLateGameSportsHandler, 60));
  ...
  server.registerTool('get_whale_trades', {
    ...
    annotations: RO,
  }, withTimestamp(getWhaleTradesHandler, 10));
  ...
  server.registerTool('get_top_funding_rates', {
    ...
    annotations: RO,
  }, withTimestamp(getTopFundingRatesHandler, 15));
  ...
  server.registerTool('get_oi_near_cap', {
    ...
    annotations: RO,
  }, withTimestamp(getOiNearCapHandler, 30));
  ...
  server.registerTool('get_price_summary', {
    ...
    inputSchema: getPriceSummarySchema,
    annotations: RO,
  }, withTimestamp(getPriceSummaryHandler, 5));

  server.registerTool('get_basic_macro', {
    title: 'Get Basic Macro',
    description: 'DXY, US10Y yield, S&P 500, gold, VIX — direct from free Yahoo Finance. Raw values + 1d change. Pro adds BTC dominance, ETH/BTC, total crypto mcap, and a RISK_ON/OFF regime classifier.',
    inputSchema: getBasicMacroSchema,
    annotations: RO,
  }, withTimestamp(getBasicMacroHandler, 30));

  server.registerTool('get_recent_news', {
    title: 'Get Recent News',
    description: 'Recent crypto news headlines matching an asset ticker, from CoinDesk + The Block + Decrypt + Cointelegraph RSS feeds. Pro adds the 1h price move that followed each headline.',
    inputSchema: getRecentNewsSchema,
    annotations: RO,
  }, withTimestamp(getRecentNewsHandler, 15));

  server.registerTool('get_simple_iv', {
    title: 'Get Simple IV',
    description: 'BTC or ETH options snapshot via free Deribit public feed: ATM implied volatility, total OI, 24h volume. Pro adds put-call skew + full term structure parsing.',
    inputSchema: getSimpleIvSchema,
    annotations: RO,
