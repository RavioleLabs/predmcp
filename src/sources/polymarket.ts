...
/**
 * Fetch positions for a single Polygon wallet, optionally filtered to one
 * market. The /positions endpoint requires `user` since the API change in
 * 2025 — there is no longer a public "all holders of market X" endpoint.
 */
export async function fetchUserPositions(
  user: string,
  opts: { conditionId?: string; minSizeUsdc?: number } = {},
): Promise<PolymarketPosition[]> {
  log.debug('fetchUserPositions');
  const url = `https://data-api.polymarket.com/positions?user=${encodeURIComponent(user)}`;
  const res = await fetch(url);
  if (!res.ok) {
    throw new Error(`Polymarket data API error: ${res.status} ${res.statusText}`);
  }
  let positions = (await res.json()) as PolymarketPosition[];
  if (!Array.isArray(positions)) positions = [];
  const min = opts.minSizeUsdc ?? 0;
  return positions.filter((p) => {
    if (opts.conditionId && p.conditionId !== opts.conditionId) return false;
    const cv = typeof p.currentValue === 'string' ? parseFloat(p.currentValue) : p.currentValue ?? 0;
    return cv >= min;
  });
}
