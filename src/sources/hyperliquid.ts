...
import { tradeBuffer, type BufferedTrade } from '../core/server/trade-buffer.js';
...
/**
 * Fetch trades directly from Hyperliquid (no buffer). Returns ~10 most recent.
 * Used by the poller to feed the buffer, and as a fallback when the buffer
 * doesn't track this coin yet.
 */
export async function fetchRecentTradesDirect(coin: string): Promise<BufferedTrade[]> {
  const trades = await getClient().recentTrades({ coin });
  return trades.map((t: { sz: string; px: string; side: string; time: number }) => ({
    time: t.time,
    px: parseFloat(t.px),
    sz: parseFloat(t.sz),
    side: t.side === 'B' ? 'B' : 'A',
...
/**
 * Whale trades for a coin. Reads from the in-memory rolling buffer (fed by
 * signal-poller every 10s for top OI coins). Falls back to a direct API
 * call for coins not currently tracked. The fallback returns only ~10 recent
 * trades, so absence of whales there does not mean none happened recently.
 */
export async function fetchWhaleTrades(coin: string, minNotionalUsdc = 50000): Promise<WhaleTrade[]> {
  log.debug('fetchWhaleTrades');

  if (tradeBuffer.has(coin)) {
    const fromBuffer = tradeBuffer.recent(coin, { minNotional: minNotionalUsdc });
    return fromBuffer.map(t => ({
      sz: t.sz.toString(),
      px: t.px.toString(),
      side: t.side,
      time: t.time,
  ...
  // Fallback for untracked coins (smaller alts) — direct API, only ~10 trades.
  const direct = await fetchRecentTradesDirect(coin);
  return direct
    .filter(t => t.notional >= minNotionalUsdc)
    .map(t => ({
      sz: t.sz.toString(),
      px: t.px.toString(),
      side: t.side,
...
export async function fetchCandles(
  ...
  log.debug('fetchCandles');
  const raw = await getClient().candleSnapshot({ coin, interval, startTime: startMs, endTime: endMs });
  return (raw ?? []).map((c: { t: number; T: number; o: string | number; h: string | number; l: string | number; c: string | number; v: string | number }) => ({
    t: c.t,
    T: c.T,
 ...
 */
export async function fetchFundingHistory(coin: string, startMs: number, endMs?: number): Promise<{ time: number; fundingRate: number }[]> {
  const raw = await getClient().fundingHistory({ coin, startTime: startMs, endTime: endMs });
  return (raw ?? []).map((h: { time: number; fundingRate: string | number }) => ({
    time: h.time,
    fundingRate: parseFloat(String(h.fundingRate)),
  }));
}
